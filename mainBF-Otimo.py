...
def solution():
    ...
    for i in range(1,1001):
        ...
        # vetor_total_racional.sort()
        #vetor_total_otimo.sort(reverse = True)
        #print(max(vetor_total_otimo))
        #print(vetor_total_otimo[0])
        ...
        # for i in range(0, M):
        #     if vetor_total_otimo[i]==max(vetor_total_otimo):
        #         print(max(vetor_total_otimo))
        #         print("Força Bruta Máximo; Dataset %d; %d; %d; %d; %d; %d; %d; %d; %d"%(dataset, (m[i][0]-o[0]), (m[i][1]-o[1]), (m[i][2]-o[2]), (m[i][3]-o[3]), (m[i][4]-s[0]), (m[i][5]-s[1]), (m[i][6]-s[2]), (m[i][7]-s[3])))


        for i in range(0, M):
            if m[i][0]>=o[0] and m[i][1]>=o[1] and m[i][2]>=o[2] and m[i][3]<=o[3] and m[i][4]>=s[0] and m[i][5]>=s[1] and m[i][6]>=s[2] and m[i][7]>=s[3]:
                if score < vetor_total_otimo[i]:
                    score = vetor_total_otimo[i]
                    escolhida = i
                    existe = 1
